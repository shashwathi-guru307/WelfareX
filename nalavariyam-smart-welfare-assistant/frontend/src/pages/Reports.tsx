...
import { reportsApi, renewalsApi } from '../services/api';
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-slate-700/50">
                {boards.map(b => (
                  <tr key={b.id} className="hover:bg-gray-50/50 dark:hover:bg-slate-700/30">
                    <td className="px-4 py-3 text-sm font-medium text-gray-900 dark:text-white">{b.name}</td>
...
function DistrictStatsReport() {
  const { data, loading, error, refetch } = useApi(() => reportsApi.getDistrictStats(), []);
  const districts: DistrictStatistic[] = data || [];

  return (
    <div className="space-y-4">
      <ReportHeader title="District-wise Statistics" count={districts.length} />
      {loading ? <LoadingSpinner /> : error ? <ErrorMessage message={error} onRetry={refetch} /> : (
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm overflow-hidden">
          <div className="overflow-x-auto">
            <table className="min-w-full">
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-slate-700/50">
                {districts.map((d, i) => (
                  <tr key={i} className="hover:bg-gray-50/50 dark:hover:bg-slate-700/30">
                    <td className="px-4 py-3 text-sm font-medium text-gray-900 dark:text-white">{d.district}</td>
