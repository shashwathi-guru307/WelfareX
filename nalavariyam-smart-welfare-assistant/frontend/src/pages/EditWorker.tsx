...
import { WELFARE_BOARDS_DATA } from '../data/welfareBoards';
...
export default function EditWorker() {
  ...
  const { data: worker, loading, error } = useApi(
    () => isCreate ? Promise.resolve(null as any) : workersApi.getById(workerId),
    [isCreate, workerId],
  );
  ...
  useEffect(() => {
    if (worker) {
      setFormData({
        ...
        board_id: worker.board_name || (worker.board_id ? String(worker.board_id) : ''),
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    setFormData(prev => {
      ...
      // Reset village when taluk changes
      if (name === 'taluk') {
        updated.village_town = '';
      }
      return updated;
  ...
  const selectedBoardData = useMemo(() => {
    ...
    return WELFARE_BOARDS_DATA.find(b => b.name === formData.board_id);
  }, [formData.board_id]);

  const natureOfWorkOptions = useMemo(() => {
    if (!selectedBoardData) return [];
    return selectedBoardData.natureOfWorks.map(w => `${w.code}: ${w.name}`);
  }, [selectedBoardData]);
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <FormSection title="Section 1: Personal Details">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <SelectField label="Gender" name="gender" value={formData.gender} onChange={handleChange} options={[...GENDER_OPTIONS]} required error={errors.gender} />
              ...
              <SelectField label="District" name="district" value={formData.district} onChange={handleChange} options={[...TAMIL_NADU_DISTRICTS]} required error={errors.district} />
              <SelectField label="Taluk" name="taluk" value={formData.taluk} onChange={handleChange} options={talukOptions} disabled={!formData.district} />
              {villageOptions.length > 0 ? (
                <SelectField label="Village / Town" name="village_town" value={formData.village_town} onChange={handleChange} options={villageOptions} disabled={!formData.taluk} />
              ) : (
              ...
              <SelectField label="Marital Status" name="marital_status" value={formData.marital_status} onChange={handleChange} options={[...MARITAL_STATUS_OPTIONS]} />
              <SelectField label="Education Level" name="education_level" value={formData.education_level} onChange={handleChange} options={[...EDUCATION_LEVELS]} />
          ...
          <FormSection title="Section 2: Worker Information">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div>
                <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Welfare Board <span className="text-red-500">*</span></label>
                <select name="board_id" value={formData.board_id} onChange={handleChange}
                  className={`w-full px-3 py-2.5 text-sm bg-gray-50 dark:bg-slate-700 border rounded-lg focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent ${errors.board_id ? 'border-red-400' : 'border-gray-200 dark:border-slate-600'} text-gray-900 dark:text-gray-100`}>
                  <option value="">— Select Board —</option>
                  {WELFARE_BOARDS_DATA.map((b, idx) => (
                    <option key={idx} value={b.name}>{b.name}</option>
                  ))}
                </select>
                {errors.board_id && <p className="text-xs text-red-500 mt-1">{errors.board_id}</p>}
              </div>
              {natureOfWorkOptions.length > 0 ? (
                <SelectField label="Nature of Work" name="nature_of_work" value={formData.nature_of_work} onChange={handleChange} options={natureOfWorkOptions} disabled={!formData.board_id} />
              ) : (
                <Field label="Nature of Work" name="nature_of_work" value={formData.nature_of_work} onChange={handleChange} placeholder={formData.board_id ? 'Select a board first' : 'Enter nature of work'} disabled={!formData.board_id} />
              )}
              ...
              <div className="flex items-center gap-3 pt-6">
                ...
                <label className="text-sm text-gray-700 dark:text-gray-300">Has Eligibility</label>
              </div>
            </div>
          </FormSection>
...
function SelectField({ label, name, value, onChange, options, required, error, disabled }: {
  label: string; name: string; value: string; onChange: (e: React.ChangeEvent<HTMLSelectElement>) => void;
  options: string[]; required?: boolean; error?: string; disabled?: boolean;
}) {
  return (
    <div>
      <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">
      ...
      <select
        ...
        onChange={onChange}
        disabled={disabled}
        className={`w-full px-3 py-2.5 text-sm bg-gray-50 dark:bg-slate-700 border rounded-lg focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent text-gray-900 dark:text-gray-100 ${error ? 'border-red-400' : 'border-gray-200 dark:border-slate-600'} ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
      >
        <option value="">{disabled ? '— Select District first —' : '— Select —'}</option>
        {options.map((opt) => (
