...
const LightPillar = ({
  ...
  useEffect(() => {
    ...
    const qualitySettings: Record<string, { iterations: number; waveIterations: number; pixelRatio: number; precision: string; stepMultiplier: number }> = {
    ...
    try {
      renderer = new THREE.WebGLRenderer({
        ...
        precision: settings.precision as any, stencil: false, depth: false
      });
    } catch {
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        for(int i = 0; i < MAX_ITER; i++) {
          ...
          col += mix(uBottomColor, uTopColor, grad) / d; t += d * STEP_MULT;
          if(t > 50.0) break;
        }
    ...
    return () => {
      ...
      rendererRef.current = null; materialRef.current = null; sceneRef.current = null;
      cameraRef.current = null; geometryRef.current = null; rafRef.current = null;
    };
  ...
  useEffect(() => {
    ...
    updateUniform('uTopColor', new THREE.Vector3(color.r, color.g, color.b));
  }, [topColor]);
  useEffect(() => {
    if (!materialRef.current) return;
    const color = new THREE.Color(bottomColor);
    updateUniform('uBottomColor', new THREE.Vector3(color.r, color.g, color.b));
  }, [bottomColor]);
  useEffect(() => updateUniform('uIntensity', intensity), [intensity]);
  useEffect(() => updateUniform('uInteractive', interactive), [interactive]);
  useEffect(() => updateUniform('uGlowAmount', glowAmount), [glowAmount]);
  useEffect(() => updateUniform('uPillarWidth', pillarWidth), [pillarWidth]);
  useEffect(() => updateUniform('uPillarHeight', pillarHeight), [pillarHeight]);
  useEffect(() => updateUniform('uNoiseIntensity', noiseIntensity), [noiseIntensity]);
  useEffect(() => {
    if (!materialRef.current) return;
    const rad = (pillarRotation * Math.PI) / 180;
    updateUniform('uPillarRotCos', Math.cos(rad));
    updateUniform('uPillarRotSin', Math.sin(rad));
  }, [pillarRotation]);

  if (!webGLSupported) {
    return <div className={`light-pillar-fallback ${className}`} style={{ mixBlendMode } as React.CSSProperties}>WebGL not supported</div>;
  }

  return <div ref={containerRef} className={`light-pillar-container ${className}`} style={{ mixBlendMode } as React.CSSProperties} />;
};
