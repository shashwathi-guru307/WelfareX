...
# Session Storage
# ============================================================
# Cloud Functions are stateless — in-memory sessions don't persist
# across invocations. We use the database for session storage.
# Falls back to in-memory for local development (SQLite).
...
def create_session(user_id: int, username: str, role: str, display_name: str) -> str:
    ...
    try:
        # Database-backed sessions (Cloud Functions / production)
        execute(
            "INSERT INTO sessions (token, user_id, username, role, display_name, expires_at) "
            "VALUES (?, ?, ?, ?, ?, ?)",
            (token, user_id, username, role, display_name, expires_at),
        )
    except Exception:
        # If sessions table doesn't exist (local dev), fall back to in-memory
...
def get_session(token: str) -> Optional[dict]:
    ...
    try:
        ...
        if row:
            ...
            # PostgreSQL returns TIMESTAMP columns as datetime objects;
            # SQLite returns strings. Normalize to datetime.
        ...
        pass  # sessions table may not exist in local dev

    # Fallback: in-memory sessions (local development with SQLite)
...
def destroy_session(token: str) -> bool:
    ...
    deleted = False
    try:
        result = execute("DELETE FROM sessions WHERE token = ?", (token,))
...
def destroy_all_sessions_for_user(user_id: int) -> int:
    """Destroy all sessions for a given user. Returns count of destroyed sessions."""
    count = 0
    try:
        # Get count before deleting
        row = fetch_one("SELECT COUNT(*) as count FROM sessions WHERE user_id = ?", (user_id,))
    ...
    tokens_to_remove = [
        t for t, s in _in_memory_sessions.items()
        if s.get('user_id') == user_id
    ]
    for t in tokens_to_remove:
        del _in_memory_sessions[t]
        count += 1
    return count


# In-memory fallback for local development (SQLite without sessions table)
