...
def get_all_workers(
    ...
    If user_id is provided, only returns workers created by that user.
    """
    conditions = ["w.is_active = 1", "w.is_archived = 0"]
    params: list = []

    # Per-user data isolation: non-admin users only see their own data
...
def create_worker(data: dict, created_by_user_id: Optional[int] = None) -> dict:
    ...
    # Add created_by_user_id if provided
    if created_by_user_id is not None:
        present_fields.append('created_by_user_id')
