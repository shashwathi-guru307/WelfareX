...
def _get_config(key: str, default: int) -> int:
    """Read a single config value from renewal_config table."""
    row = fetch_one(
...
def get_renewal_breakdown() -> dict:
    """
    ...
    active = fetch_one(
        """SELECT COUNT(*) as count FROM registrations r
           ...
           WHERE r.validity_date >= ? AND w.is_active = 1""",
        ((date.today() + timedelta(days=thresholds["alert_days"] + 1)).isoformat(),)
    )

    expiring_soon = fetch_one(
        """SELECT COUNT(*) as count FROM registrations r
           JOIN workers w ON r.worker_id = w.id
           WHERE r.validity_date >= ? AND r.validity_date <= ? AND w.is_active = 1""",
        (today, (date.today() + timedelta(days=thresholds["alert_days"])).isoformat())
    )
    ...
    # Workers with registration but missing validity_date
    missing_date = fetch_one(
        """SELECT COUNT(*) as count FROM registrations r
           JOIN workers w ON r.worker_id = w.id
           WHERE (r.validity_date IS NULL OR r.validity_date = '')
           AND w.is_active = 1"""
    )

    # Critical urgency (7 days or less)
    critical = fetch_one(
        """SELECT COUNT(*) as count FROM registrations r
           JOIN workers w ON r.worker_id = w.id
           WHERE r.validity_date >= ? AND r.validity_date <= ? AND w.is_active = 1""",
        (today, (date.today() + timedelta(days=thresholds["critical_days"])).isoformat())
    )
    ...
    high = fetch_one(
        """SELECT COUNT(*) as count FROM registrations r
           ...
           WHERE r.validity_date > ? AND r.validity_date <= ? AND w.is_active = 1""",
        (
            (date.today() + timedelta(days=thresholds["critical_days"])).isoformat(),
            (date.today() + timedelta(days=thresholds["high_days"])).isoformat(),
        )
    )
